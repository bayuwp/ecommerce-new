...
<script>
    $(document).ready(function() {
        ...
        window.submitCheckout = function() {
            ...
            const requiredFields = ['origin_province', 'origin_city', 'destination_province', 'destination_city', 'weight', 'courier', 'address_detail'];
            let hasError = false;
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        $('#checkout-form').hide();
                        showResultForm(response.data); // Menampilkan opsi ongkir
                    } else {
        ...
        function showResultForm(data) {
            ...
            data.forEach((courier, courierIndex) => {
                const courierElement = $(`<div><h4>${courier.name}</h4></div>`);
            ...
            submitButton.on('click', processPayment);
        ...
        function processPayment() {
            ...
            if (!selectedOption) {
                alert('Silakan pilih salah satu opsi ongkos kirim.');
                return;
            }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Update subtotal ketika kuantitas diubah
